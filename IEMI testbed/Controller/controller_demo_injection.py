...
address="127.0.0.1" # Change this to the target address
i2cbus = SMBus(1)
...
print('IMU test gyroscope readings')
time.sleep(2)
for i in range(0,100):
    gyro = i2cbus.read_word_data(IMU_address,GYRO_XOUT_H)
    print('gyro: '+ str(gyro))
print('Starting IEMI Attack')
time.sleep(2)
write_done = False
t1 = time.time()
# for ix in range(50):
while(True):
    ...
    try:
        ...
        s.connect((address, 8040))
        time.sleep(0.0054)
        i2cbus.write_byte_data(IMU_address, PWR_MGMT_1, 0x01)
        write_done = True
        if write_done:
            sampling_rate = i2cbus.read_byte_data(IMU_address,PWR_MGMT_1)
...
sampling_rate = i2cbus.read_byte_data(IMU_address,PWR_MGMT_1)
print('Register value: '+str(sampling_rate))
