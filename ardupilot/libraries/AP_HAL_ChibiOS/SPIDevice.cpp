...
bool SPIDevice::do_transfer(const uint8_t *send, uint8_t *recv, uint32_t len)
{
    ...
    osalSysLock();
////  evaluating traffic on SPI level. Only works correctly for kakuteh7mini-nand
////  kakute uses 3 SPI devices. SPI4 -> bmi270, SPI1 -> dataflash, SPI2 -> osd
////  We are interested in SPI4. SPI4 TXDR is at 0x40013420 for stm32h743
    SPIDriver *spointer = spi_devices[device_desc.bus].driver;
    uint32_t curr_txdr_address = (uint32_t)(&spointer->spi->TXDR);
    uint32_t stm32h7_hw_txdr = 0x40013420;
//    Equal data is written/read from TXDR and RXDR. Doing for one only.
//    uint32_t curr_rxdr_address = (uint32_t)(&spointer->spi->RXDR);
//    uint32_t stm32h7_hw_rxdr = 0x40013430;

    if (curr_txdr_address == stm32h7_hw_txdr) {
        hal.util->persistent_spi_data.spi_txn_count++;
    }

    hal.util->persistent_data.spi_count++;
    if (send == nullptr) {
        if(curr_txdr_address == stm32h7_hw_txdr){
            hal.util->persistent_spi_data.spi_len_count += len;
        }
