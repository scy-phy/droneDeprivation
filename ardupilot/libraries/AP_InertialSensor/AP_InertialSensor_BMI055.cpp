...
bool AP_InertialSensor_BMI055::accel_init()
{
    ...
    if (!dev_accel->write_register(REGA_ACCD_HBW, 0x00, true)) {
        goto failed;
    }
...
{
    //Frequency change attack
    // unsigned char frequencies[8] = {0x0F,0x0E,0x0D,0x0C,0x0B,0x0A,0x09,0x08};
    // global_counter_acc ++;  
    // // //setup filter bandwidth 
    // if(global_counter_acc%10000 == 0){
    //     counter_acc++;
    //     dev_accel->write_register(REGA_PMU_BW, frequencies[counter_acc%8], true);
    // }

    // AP::logger().Write("AC", "TimeUS,Ground_Truth", "QI",
    //                                    AP_HAL::micros64(),
    //                                    frequencies[counter_acc%8]);

    // Frequency attack only once
    // global_counter_acc ++;
   
    // if (global_counter_acc%10000 == 0){
    //     ground_truth = 1;
    //     GCS_SEND_TEXT(MAV_SEVERITY_WARNING,"Attack starts");
    //     dev_accel->write_register(REGA_PMU_BW, 0x09, true);
    // }
    // //stop
    // if (global_counter_acc%10000 == 2500){
    //     dev_accel->write_register(REGA_PMU_BW, 0x0F , true);
    //     GCS_SEND_TEXT(MAV_SEVERITY_WARNING,"Attack stops");
    //     ground_truth = 0;
    // }

    // AP::logger().Write("GT", "TimeUS,Ground_Truth", "QI",
    //                                    AP_HAL::micros64(),
    //                                    ground_truth);

    //enable suspend mode ATTACK
    // if(global_counter_acc%10000 == 0){
    //     ground_truth = 1;
    //     dev_accel->write_register(REGA_PMU_LPW, 0x80, true);
    //     //dev_accel->write_register(0x12, 0x40, true);
    //     // hal.scheduler->delay(1);
    // }
    // //disable_suspend
    // if(global_counter_acc%10000 == 2500){
    //     dev_accel->write_register(REGA_PMU_LPW, 0, true);
    //     //hal.scheduler->delay(1);
    //     ground_truth = 0;
    // }
    // AP::logger().Write("GT", "TimeUS,Ground_Truth", "QI",
    ...
    for (uint8_t i = 0; i < num_frames; i++) {
        ...
        AP::logger().Write("RAC", "TimeUS,ACCx,ACCy,ACCz", "Qfff",
                                       AP_HAL::micros64(),
...
{   
    // //FREQUENCY ATTACK
    // unsigned char frequencies[8] = {0x81,0x82,0x83,0x84,0x85,0x86,0x87,0x87};
    // global_counter_gyro ++;  
    // // setup filter bandwidth 
    // if(global_counter_gyro%10000 == 0){
    //     counter_gyro++;
    //     dev_gyro->write_register(REGG_BW, frequencies[counter_gyro%8], true);
    // }
    ...
    // if (global_counter_gyro%10000 == 0){
    //     dev_gyro->write_register(REGG_BW, 0x87, true);
    // }
    // //stop
    // if (global_counter_gyro%10000 == 2500){
    //     dev_gyro->write_register(REGG_BW, 0x81, true);
    // }
    ...
    //     dev_gyro->write_register(REGG_LPM1, 0x80, true);
    //     //dev_gyro->write_register(0x12, 1, true);
    // }
    // //disable_suspend
    // if(global_counter_gyro%10000 == 2500){
    //     dev_gyro->write_register(REGG_LPM1, 0, true);
    // }
    ...
    for (uint8_t i = 0; i < num_frames; i++) {
        ...
        AP::logger().Write("RGY", "TimeUS,GYROx,GYROy,GYROz", "Qfff",
                                       AP_HAL::micros64(),
