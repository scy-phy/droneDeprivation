...
bool AP_InertialSensor_Invensense::_accumulate(uint8_t *samples, uint8_t n_samples)
{
    for (uint8_t i = 0; i < n_samples; i++) {
        ...
        // AP::logger().Write("RAC", "TimeUS,ACCx", "Qf",
        //                                AP_HAL::micros64(),
        //                                accel.x);

        // AP::logger().Write("RGY", "TimeUS,GYROx", "Qf",
        //                                AP_HAL::micros64(),
        //                                gyro.x);
...
void AP_InertialSensor_Invensense::set_frequencies(unsigned char frequency){
    GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "Attack Starts");
    //_register_write(MPUREG_CONFIG, frequency, true);
    _register_write(MPUREG_SMPLRT_DIV, frequency, true);
}

void AP_InertialSensor_Invensense::_read_fifo()
{
    //FREQUENCY ATTACK
    //unsigned char frequencies[7] =   {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06};
    unsigned char frequencies[6] =   {MPUREG_SMPLRT_1000HZ, MPUREG_SMPLRT_500HZ, MPUREG_SMPLRT_250HZ, MPUREG_SMPLRT_200HZ, MPUREG_SMPLRT_100HZ, MPUREG_SMPLRT_50HZ};
    global_counter ++;
    if  (global_counter%10000 == 0){
        attack_counter++;
        set_frequencies(frequencies[attack_counter%6]);
    }
    ...
    //     ground_truth = 100;
    //     /* sleep device */
    //     _register_write(MPUREG_PWR_MGMT_1, 0x40);
    //     hal.scheduler->delay(100);
    // }
    // //ENABLE SENSOR again
    // if (global_counter%10000 == 2500){
    //     // Turn on in low noise GYRO and ACC
    //     _register_write(MPUREG_PWR_MGMT_1, 0x80);
    //     hal.scheduler->delay(100);
