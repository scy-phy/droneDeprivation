...
bool AP_InertialSensor_Invensensev3::accumulate_samples(const FIFOData *data, uint8_t n_samples)
{
    for (uint8_t i = 0; i < n_samples; i++) {
        ...
        AP::logger().Write("RAC2", "TimeUS,ACCx", "Qf",
                                       AP_HAL::micros64(),
                                       accel.x);

        AP::logger().Write("RGY2", "TimeUS,GYROx", "Qf",
                                       AP_HAL::micros64(),
                                       gyro.x);
...
void AP_InertialSensor_Invensensev3::set_frequencies(unsigned char frequency_acc, unsigned char frequency_gyro){
    GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "Attack Starts");
    ground_truth = 100;
    register_write(INV3REG_GYRO_CONFIG0, frequency_gyro);
    register_write(INV3REG_ACCEL_CONFIG0, frequency_acc);
}
...
{
    //FREQUENCY ATTACK
    // unsigned char frequencies_acc[7] =   {0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B};
    // unsigned char frequencies_gyro[7] =  {0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B};
    // global_counter ++;
    // if  (global_counter%10000 == 0){
    //     GCS_SEND_TEXT(MAV_SEVERITY_WARNING,"Attack starts");
    //     attack_counter++;
    //     set_frequencies(frequencies_acc[attack_counter%7], frequencies_gyro[attack_counter%7]);
    // }
    // AP::logger().Write("AC2", "TimeUS,Ground_Truth", "QI",
    //                                    AP_HAL::micros64(),
    //                                    attack_counter%7);
    ...
    //     GCS_SEND_TEXT(MAV_SEVERITY_WARNING,"Attack starts");
    //     ground_truth = 1;
    //     set_frequencies(0x0B, 0x0B);
    // }

    // if  (global_counter%10000 == 2500){
    //     attack_counter++;
    //     set_frequencies(0x05, 0x05);
    //     ground_truth = 0;
    //     GCS_SEND_TEXT(MAV_SEVERITY_WARNING,"Attack stop");
    // }
    // AP::logger().Write("GT2", "TimeUS,Ground_Truth", "QI",
    //                                    AP_HAL::micros64(),
    //                                     ground_truth);
     

    // // //ATTACK sending sensor to off mode
    // global_counter ++;
    // if (global_counter%10000 == 0){
    //     GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "Attack Starts");
    //     // Turn off GYRO and ACC
    //     ground_truth = 100;
    //     register_write(INV3REG_PWR_MGMT0, 0x0);
    //     hal.scheduler->delay_microseconds(300);
    // }
    // //ENABLE SENSOR again
    // if (global_counter%10000 == 2500){
    //     // Turn on in low noise GYRO and ACC
