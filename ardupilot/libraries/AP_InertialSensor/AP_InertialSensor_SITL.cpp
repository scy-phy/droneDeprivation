...
void AP_InertialSensor_SITL::timer_update(void)
{
    ...
    if (SIMULATE_INVENSENSE_SUSPEND){
            accel.x = 0.2477;  
            accel.y = 68.9905;
            accel.z = -0.3031;
            gyro.x = 0.0254;  
    ...
    if (next_accel_sample == 0) {
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "set attack timer");
        attack_start = now + 100000000UL;
    }
    // // frequency attack
    // if (now >= attack_start && attack_start > attack_stop && !attack_started){
    //     GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "Attack Starts");
    //     attack_started = true;
    //     accel_sample_hz = 150; //12.5 bmi 270;
    //     gyro_sample_hz = 150; //12.5 bmi 270;
    //     attack_stop = now + 1000000UL;
    // }

    // if (now >= attack_stop && attack_started){
        // accel_sample_hz = 1000;
        // gyro_sample_hz = 1000;
...
#endif
                if (attack_started && (SIMULATE_BMI270_SUSPEND||SIMULATE_INVENSENSE_SUSPEND)){
