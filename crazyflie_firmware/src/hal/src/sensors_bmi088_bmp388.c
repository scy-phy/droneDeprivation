...
{
  //DEBUG_PRINT("tick: %d\n",(int)tick);
  // if (tick == 30000 && !attack_start){
  //   DEBUG_PRINT("suspend attack start...\n");
  //   attack_start = true;
  //   bmi088Dev.accel_cfg.power = BMI088_ACCEL_PM_SUSPEND;
  //   bmi088_set_accel_power_mode(&bmi088Dev);
  //   //bmi088Dev.delay_ms(10);
  //   //uint8_t three = 0x03;
  //   //bmi088_set_accel_regs(0x7C, &three, BMI088_ONE, &bmi088Dev);
  //   bmi088Dev.gyro_cfg.power = BMI088_GYRO_PM_SUSPEND;
  //   bmi088_set_gyro_power_mode(&bmi088Dev);
  //   //uint8_t eighty = 0x80;
  //   //bmi088_set_gyro_regs(0x11, &eighty, BMI088_ONE, &bmi088Dev);
  //   //bmi088Dev.delay_ms(10);
  //   DEBUG_PRINT("executed code\n");
  // }
  // if (tick == 30001 && attack_start){
  //   attack_start = false;
  ...
  //   bmi088_set_gyro_power_mode(&bmi088Dev);
  //   //bmi088Dev.delay_ms(10);
  //   DEBUG_PRINT("suspend attack stop...\n");
  // }

  // if (attack_start) {
  //   DEBUG_PRINT("read G before\n");
  // }
  ...
  // if (attack_start) {
  //   DEBUG_PRINT("read M and B\n");
  // }
  // sensors->interruptTimestamp = sensorData.interruptTimestamp;
  // if (attack_start) {
  //   DEBUG_PRINT("tick: %d\n",(int)tick);
  //   DEBUG_PRINT("read timestamp\n");
  // }

  if (tick == 30000 && !attack_start){
    DEBUG_PRINT("tick: %d\n",(int)tick);
    DEBUG_PRINT("frequency attack start...\n");
    attack_start = true;
    bmi088Dev.gyro_cfg.odr = BMI088_GYRO_BW_32_ODR_100_HZ;
    bmi088_set_gyro_meas_conf(&bmi088Dev);
    bmi088Dev.accel_cfg.odr = BMI088_ACCEL_ODR_12_5_HZ;
    bmi088_set_accel_meas_conf(&bmi088Dev);
  }

  if (tick == 30040 && attack_start){
    DEBUG_PRINT("tick: %d\n",(int)tick);
    DEBUG_PRINT("frequency attack stop...\n");
...
static void sensorsTask(void *param)
{
  ...
  while (1)
  { 
    if (attack_start) { DEBUG_PRINT("pull read before\n");}
    if (pdTRUE == xSemaphoreTake(sensorsDataReady, portMAX_DELAY) || attack_start)
    {
      if (attack_start) {DEBUG_PRINT("pull read after\n");}
